...
const EMAIL_EVENTS: EventMeta[] = [
  ...
  { key: "emailAdminRejection", label: "Admin Rejection Alert", description: "Notifies all admins when an operator rejects completed work (especially disputes).", recipient: "All admins" },
...
export default function SettingsPage() {
  ...
  if (loading) {
    return (<div className="flex items-center justify-center py-24"><Loader2 className="w-6 h-6 animate-spin text-slate-400" /></div>);
  }

  return (
    <div className="max-w-3xl mx-auto py-8 px-4 space-y-8">
      <div>
        <h1 className="text-2xl font-bold text-slate-900 flex items-center gap-2"><Settings className="w-6 h-6 text-blue-600" />Settings</h1>
        <p className="mt-1 text-sm text-slate-500">Control which system emails are sent and configure global BCC.</p>
      </div>
      {toast && (
        <div className={`flex items-center gap-2 px-4 py-3 rounded-md text-sm ${toast.type === "success" ? "bg-emerald-50 text-emerald-800" : "bg-red-50 text-red-800"}`}>
      ...
      <section className="rounded-lg border border-slate-200 bg-white">
        <div className="px-6 py-4 border-b border-slate-200">
          <h2 className="text-base font-semibold text-slate-800 flex items-center gap-2"><Mail className="w-4 h-4 text-slate-500" />Email Notifications</h2>
        ...
        <div className="divide-y divide-slate-100">
          {EMAIL_EVENTS.map((evt) => (
            <div key={evt.key} className="flex items-center justify-between px-6 py-4 gap-4">
              ...
              <button type="button" role="switch" aria-checked={settings[evt.key] as boolean} onClick={() => toggleEvent(evt.key)} disabled={saving}
                className={`relative inline-flex h-6 w-11 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 disabled:opacity-50 ${settings[evt.key] ? "bg-blue-600" : "bg-slate-200"}`}>
                <span className={`pointer-events-none inline-block h-5 w-5 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out ${settings[evt.key] ? "translate-x-5" : "translate-x-0"}`} />
              </button>
            </div>
          ))}
        </div>
      ...
      <section className="rounded-lg border border-slate-200 bg-white">
        <div className="px-6 py-4 border-b border-slate-200">
          <h2 className="text-base font-semibold text-slate-800 flex items-center gap-2"><Copy className="w-4 h-4 text-slate-500" />Global BCC</h2>
        ...
        <div className="px-6 py-4 space-y-4">
          <div className="flex items-center justify-between">
            ...
            <button type="button" role="switch" aria-checked={settings.bccEnabled} onClick={toggleBcc} disabled={saving}
              className={`relative inline-flex h-6 w-11 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 disabled:opacity-50 ${settings.bccEnabled ? "bg-blue-600" : "bg-slate-200"}`}>
              <span className={`pointer-events-none inline-block h-5 w-5 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out ${settings.bccEnabled ? "translate-x-5" : "translate-x-0"}`} />
            </button>
          </div>
          <div>
            <label htmlFor="bccAddresses" className="block text-sm font-medium text-slate-700 mb-1">BCC Email Addresses</label>
            <input id="bccAddresses" type="text" value={settings.bccAddresses} onChange={(e) => setSettings((prev) => ({ ...prev, bccAddresses: e.target.value }))} onBlur={() => save({ bccAddresses: settings.bccAddresses })} placeholder="admin@example.com, manager@example.com" disabled={saving} className="w-full border border-slate-300 rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 disabled:opacity-50" />

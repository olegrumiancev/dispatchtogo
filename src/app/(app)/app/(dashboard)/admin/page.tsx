...
export default async function AdminDashboardPage() {
  ...
  const [
    ...
    prisma.serviceRequest.count({ where: { status: { in: ["SUBMITTED", "READY_TO_DISPATCH"] }, job: null } }),
    prisma.job.count({ where: { completedAt: null } }),
    prisma.job.count({ where: { completedAt: { gte: firstOfMonth, not: null } } }),
    prisma.serviceRequest.count(),
    prisma.vendor.count({ where: { isActive: true } }),
    prisma.organization.count(),
    prisma.serviceRequest.count({ where: { urgency: "EMERGENCY", status: { notIn: ["COMPLETED", "VERIFIED", "CANCELLED"] } } }),
    prisma.serviceRequest.count({ where: { createdAt: { gte: firstOfMonth } } }),
  ]);

  const stats = [
    { label: "Pending Dispatch", value: pendingDispatch, icon: Send, color: "bg-orange-100 text-orange-600", highlight: pendingDispatch > 0 },
    { label: "Active Jobs", value: activeJobs, icon: Briefcase, color: "bg-blue-100 text-blue-600", highlight: false },
    { label: "Completed This Month", value: completedThisMonth, icon: CheckCircle, color: "bg-emerald-100 text-emerald-600", highlight: false },
    { label: "Emergencies", value: emergencyRequests, icon: AlertTriangle, color: "bg-red-100 text-red-600", highlight: emergencyRequests > 0 },
    { label: "Requests This Month", value: requestsThisMonth, icon: TrendingUp, color: "bg-purple-100 text-purple-600", highlight: false },
    { label: "Total Requests", value: totalRequests, icon: ClipboardList, color: "bg-gray-100 text-gray-600", highlight: false },
  ];

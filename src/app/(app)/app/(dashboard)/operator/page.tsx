                  ...
                  <tr key={req.id} className={`hover:bg-gray-50 transition-colors ${req.hasNewActivity ? "bg-amber-50 hover:bg-amber-100" : ""}`}>
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-1.5">
                        {req.hasNewActivity && (
                          <span title="New vendor activity" className="inline-block h-2 w-2 rounded-full bg-amber-400 animate-pulse flex-shrink-0" />
                        )}
                        <Link href={`/app/operator/requests/${req.id}`} className="text-sm font-medium text-blue-600 hover:text-blue-700">
                          {req.referenceNumber}
                        </Link>
                      </div>
                    </td>
                    <td className="px-6 py-4 text-sm text-gray-700 max-w-[160px] truncate">{req.property.name}</td>
                    <td className="px-6 py-4 text-sm text-gray-500 hidden md:table-cell">{getCategoryLabel(req.category)}</td>
                    ...
                    <td className="px-6 py-4 text-sm text-gray-500 hidden lg:table-cell">{formatDate(req.createdAt)}</td>
                  </tr>
                ))}
              </tbody>
            </table>

...
export default async function PropertiesPage({
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="overflow-x-auto">
          {properties.length === 0 ? (
            <div className="px-6 py-16 text-center">
              ...
              <p className="text-sm text-gray-400 mt-1">Add a property to start managing service requests.</p>
            </div>
          ) : (
                  ...
                  {([
                    ...
                    return (
                      <th key={col} className={`text-left px-6 py-3 text-xs font-medium text-gray-500 uppercase tracking-wider ${cls}`}>
                        <Link href={sortUrl(col)} className={`inline-flex items-center gap-1 hover:text-gray-800 transition-colors ${active ? "text-gray-800" : ""}`}>
                          {label}
              ...
              <tbody className="divide-y divide-gray-200">
                {properties.map((property) => (
                  <tr key={property.id} className="hover:bg-gray-50 transition-colors">
                    <td className="px-6 py-4"><p className="text-sm font-medium text-gray-900">{property.name}</p></td>
                    <td className="px-6 py-4 hidden md:table-cell">
                      <p className="text-sm text-gray-600 max-w-[200px] truncate">{property.address ?? <span className="text-gray-400">—</span>}</p>
                    </td>
                    <td className="px-6 py-4 hidden lg:table-cell">
                      <p className="text-sm text-gray-500 max-w-[220px] truncate">{property.description ?? <span className="text-gray-400">—</span>}</p>
                    </td>
                    <td className="px-6 py-4 text-center hidden sm:table-cell">
                      <Link href={`/app/operator/requests?property=${property.id}`} className={`text-sm font-semibold ${property._count.serviceRequests > 0 ? "text-blue-600 hover:text-blue-700" : "text-gray-400"}`}>
                        {property._count.serviceRequests}
                      </Link>
                    </td>
                    <td className="px-6 py-4">
                      <Badge variant={property.isActive ? "bg-emerald-100 text-emerald-700" : "bg-gray-100 text-gray-500"}>
                    ...
                    <td className="px-6 py-4 text-right">
                      <div className="flex items-center justify-end gap-1">
                        <Link href={`/app/operator/requests?property=${property.id}`}>
                          <Button variant="ghost" size="sm"><Eye className="w-4 h-4" /><span className="hidden sm:inline">Requests</span></Button>
                        </Link>
                        <PropertyActions propertyId={property.id} propertyName={property.name} isActive={property.isActive} activeRequestCount={property.serviceRequests.length} />
                      </div>
                    </td>
                  </tr>
                ))}
      ...
      <PaginationControls page={page} totalPages={totalPages} basePath="/app/operator/properties" extraParams={{ sortBy, sortDir }} total={total} pageSize={PAGE_SIZE} />
    </div>
  );
}

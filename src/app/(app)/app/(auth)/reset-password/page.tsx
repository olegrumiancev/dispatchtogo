...
function ResetPasswordForm() {
    ...
    return (
      <div className="w-full max-w-md">
        <div className="bg-white rounded-2xl shadow-2xl p-8 text-center">
          ...
          <Link href="/app/forgot-password" className="text-blue-600 hover:text-blue-700 font-medium text-sm">Request a new reset link</Link>
        </div>
      </div>
    );
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) { setError(data.error || "Something went wrong."); } else { setSuccess(true); }
    ...
    return (
      <div className="w-full max-w-md">
        <div className="bg-white rounded-2xl shadow-2xl p-8 text-center">
          ...
          <Link href="/app/login" className="inline-flex items-center gap-2 bg-blue-600 text-white px-6 py-3 rounded-lg font-medium hover:bg-blue-700 transition-colors">Sign In</Link>
        </div>
      </div>
    );
  }
    ...
    <div className="w-full max-w-md">
      ...
      <div className="bg-white rounded-2xl shadow-2xl p-8">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <Input label="New Password" type="password" placeholder="••••••••" value={password} onChange={(e) => setPassword(e.target.value)} required autoComplete="new-password" />
          <Input label="Confirm New Password" type="password" placeholder="••••••••" value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} required autoComplete="new-password" />

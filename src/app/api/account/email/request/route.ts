...
export async function POST(request: NextRequest) {
  ...
  const confirmationTemplate = await renderEmailTemplate("emailChangeConfirmNew", {
    name: currentUser.name || "there",
    currentEmail: currentUser.email,
    newEmail,
    confirmUrl,
  });
  const confirmationEmail = await sendEmail(newEmail, confirmationTemplate.subject, confirmationTemplate.html);
  ...
  renderEmailTemplate("emailChangeRequestedOld", {
    name: currentUser.name || "there",
    currentEmail: currentUser.email,
    newEmail,
  }).then((template) =>
    sendEmail(currentUser.email, template.subject, template.html)
  ).catch((error) => {
